...
while kohta<n:
    if(lajittelemattomat[kohta].sukupuoli !='mies' and lajittelemattomat[kohta] !=null): #On siis nainen tai muu
        if(i%2==0 and j%2==0 and poyta[i][j]==null): #on siis parillinen ja parillinen
            lajittelemattomat[kohta]=poyta[i][j]
            i+=1
        elif(i%2==0 and j%2==1 and poyta[i][j]==null): #on siis parillinen ja pariton
            lajittelemattomat[kohta]=poyta[i][j]
            j+=1
        elif(poyta[i][j] != null):
            if j==2:
    ...
    if(lajittelemattomat[kohta].sukupuoli!='nainen'): #On siis mies tai muu
        if (i % 2 == 1 and j % 2 == 1 and poyta[i][j] == null):  # on siis parillinen ja parillinen
            lajittelemattomat[kohta] = poyta[i][j]
            j += 1
